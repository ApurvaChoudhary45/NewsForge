...
const Page = () => {
    ...
    useEffect(() => {
        const fetched = async () => {
            setloading(true)
            try {
                const newly = await fetch('/api/getpost')
    ...
    return (
        <>
            <div>
                <div >
                    <nav className={`w-full ${darkMode ? ('bg-black/80') : ('bg-white')} backdrop-blur-md text-red-600 flex items-center justify-between px-6 py-4 shadow-md fixed top-0 z-50`}>
            ...
            <div className={`relative z-40 flex justify-center items-center py-20 px-2 ${darkMode ? ('bg-black/80') : ('bg-white')}`}>
